...
- **语义切片策略**：基于 Recursive Character Text Splitter 的智能文本分块，可配置 chunk_size（默认 500）与 chunk_overlap（默认 50），平衡上下文完整性与检索精度
- **上下文验证**：在检索结果返回前进行语义相关性校验，确保引入的知识与问题高度相关
- **双知识库架构**：支持火山引擎云端知识库与本地 ChromaDB 向量数据库的无缝切换
...
- 🎤 **实时语音识别**：基于浏览器原生能力，面试官提问即时转文字
- 📚 **双知识库支持**：火山引擎云端知识库 / 本地 ChromaDB，自由切换，降低成本
- 📤 **文档上传**：支持 PDF、Word、TXT 等多种格式文档上传至本地知识库
- 🔪 **智能切片**：可配置的文本分块参数，优化检索效果
